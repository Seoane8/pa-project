...
describe('Rendering InscriptionForm component', () => {
  ...
  describe('Trying to inscribe for the sport test', () => {
    let cardNumberInput
    let inscribeButton
    ...
    /*describe('by inserting wrong entries', () => {
      test('when not insert card number, show validation error', () => {
        fireEvent.click(inscribeButton)
  
        const validationError = component.getByText(messages_en['project.global.validator.cardNum'])
        expect(validationError).toBeVisible()
      })
  
      test('when insert short card number, show validation error', () => {
        const cardNumber = generateRandomCardNumber(13)
    ...
    describe('by inserting correct entries', () => {
